...
if crank == root
    Test.@test isapprox(out_map.i.pixels, alm2map(test_alm, NSIDE).i.pixels) #test against Healpix transform
    Test.@test isapprox(out_map.q.pixels, alm2map(test_alm, NSIDE).q.pixels) #test against Healpix transform
    Test.@test isapprox(out_map.u.pixels, alm2map(test_alm, NSIDE).u.pixels) #test against Healpix transform
end
